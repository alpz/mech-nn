...
def plot_logistic(params, act_rs, act_rks, save_path):
    ...
    #ax.plot(act_rs, label='GT Bias')
    ax.plot(params[:,0], label='Learned bias', alpha=0.9)

    ax.plot(act_rs, label='GT R')
    ax.plot(params[:,1], label='Learned R', alpha=0.9)

    ax.plot(act_rks, label='GT -R/K')
    ax.plot(params[:,2], label='Learned -R/K', alpha=0.9)
