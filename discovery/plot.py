...
def plot_cr(data, save_path):
    f = plt.figure(figsize=(6, 4))
    plt.plot(data, label='Trajectory')
    plt.plot(data[0], "ko", label="Initial condition", markersize=8)
    plt.legend()
    #plt.show()
    print(f'Saving plot {save_path}')
    plt.savefig(save_path, bbox_inches='tight')
...
#    """ params[...,2]  r, r/k"""
#    f = plt.figure(figsize=(6, 4))
#    plt.plot(act_rs, label='GT R')
#    plt.plot(params[:,0], label='Learned R')
#
#    plt.plot(act_rks, label='GT -R/K')
#    plt.plot(params[:,1], label='Learned -R/K')
#
#    plt.legend()
#    #plt.show()
#    print(f'Saving plot {save_path}')
